...
extern crate csv;   // To use csv reading, make sure to add the dependency `csv = "1.2"` in Cargo.toml
use csv::Reader;
use std::error::Error;
use std::io::Write;
use std::path::Path;
...
pub fn matrix_to_bitmatrix_from_csv(file_path: &str, nr_data_block: usize, nr_parity_block: usize) -> BitMatrix {
    let mut inner: Vec<Vec<bool>> = Vec::new();
    let mut rdr = ReaderBuilder::new()
        .has_headers(false)    // If headers are not needed, set it to false
        .from_path(file_path)
        .unwrap();

    for result in rdr.records() {
        if let Ok(record) = result {
            let row: Vec<bool> = record.iter().map(|e| e == "1").collect();
            inner.push(row);
        }
    }
